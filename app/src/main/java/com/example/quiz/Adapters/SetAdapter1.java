...
import com.example.quiz.Activities.Pontuacao1Activity;
import com.example.quiz.Activities.Questoes1Activity;
...
//Adapter HTML
public class SetAdapter1 extends RecyclerView.Adapter<SetAdapter1.viewHolder> {

    Context context;
    ArrayList<SetModel> list;
    List<DocumentSnapshot> listDocuments;

    public SetAdapter1(Context context, ArrayList<SetModel> list, List<DocumentSnapshot> listDocuments) {
                ...
                for(DocumentSnapshot dc: listDocuments){
                    if(model.getSetNome().toLowerCase().equals(dc.getId().toLowerCase())){
                        if(dc.getBoolean("status")){
                            ...
                            Intent intent = new Intent(context, Pontuacao1Activity.class);
                            intent.putExtra("total", Integer.parseInt(String.valueOf(dc.getLong("total"))));
                            intent.putExtra("fase1", model.getSetNome());
                            intent.putExtra("pontos", Integer.parseInt(String.valueOf(dc.getLong("respCertas"))));
                            context.startActivity(intent);
                        }else {
                            Intent intent = new Intent(context, Questoes1Activity.class);
                            intent.putExtra("fase1", model.getSetNome());
                            context.startActivity(intent);
                        }
                    }
