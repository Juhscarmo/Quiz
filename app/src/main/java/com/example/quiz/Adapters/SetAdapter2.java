...
import com.example.quiz.Activities.Pontuacao2Activity;
import com.example.quiz.Activities.Questoes2Activity;
...
public class SetAdapter2 extends RecyclerView.Adapter<SetAdapter2.viewHolder> {
    ...
    public SetAdapter2(Context context, ArrayList<SetModel> list, List<DocumentSnapshot> listDocuments) {
                ...
                for(DocumentSnapshot dc: listDocuments){
                    if(model.getSetNome().toLowerCase().equals(dc.getId().toLowerCase())){
                        if(dc.getBoolean("status")){
                            ...
                            Intent intent = new Intent(context, Pontuacao2Activity.class);
                            intent.putExtra("total", Integer.parseInt(String.valueOf(dc.getLong("total"))));
                            intent.putExtra("fase2", model.getSetNome());
                            intent.putExtra("pontos", Integer.parseInt(String.valueOf(dc.getLong("respCertas"))));
                            context.startActivity(intent);
                        }else {
                            Intent intent = new Intent(context, Questoes2Activity.class);
                            intent.putExtra("fase2", model.getSetNome());
                            context.startActivity(intent);
                        }
                    }
